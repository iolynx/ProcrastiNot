import '@tensorflow/tfjs-backend-cpu'; // <-- ADD THIS LINE
...
//   console.log('Extension Installed. Pre-loading model...');
//   // We call getModel here to start the loading process in the background
//   getModel();
// });

// This function ensures the model is loaded before we use it
async function getModel() {
  if (model === null) {
    console.log('Model is not loaded, loading now...');
    const startTime = performance.now();
    model = await use.load();
    const loadTime = (performance.now() - startTime).toFixed();
...
async function checkActiveTabNow() {
  chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
    ...
    if (activeTab) {
      console.log(
        `Checking  active tab: ${activeTab.title}, ID: ${activeTab.id}`
      );
      await chrome.scripting.executeScript({
  ...
  chrome.tabs.sendMessage(tabId, { action: 'SENDINFO' }, async (response) => {
    if (chrome.runtime.lastError) {
      ...
      console.log(
        '%cINFO:',
        'color:aquamarine',
        'Page embeddings took',
        pageEmbeddingsTime.toFixed(2),
        'ms.'
      );

      let predictionStartTime = performance.now();
      const prediction = await analyzeEmbeddings(
        titleEmbeddings.arraySync()[0],
        textEmbeddings.arraySync()[0]
      );
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'addPhrase') {
  ...
  } else if (message.type === 'removePhrase') {
    ...
    sendResponse({ success: true });
  } else if (message.type === 'getPhrases') {
...
async function analyzeEmbeddings(titleEmbeddings, contentEmbeddings) {
  ...
  const focusEmbeddings = await Promise.all(
    focusPhrasesFinal.map(async (text) => await embedText(text))
  );
  const distractionEmbeddings = await Promise.all(
    distractionExamples.map(async (text) => await embedText(text))
  );

  // Calculate maximum similarity for both title and content embeddings
  const maxFocusSimilarityTitle = maxSimilarity(
    titleEmbeddings,
    focusEmbeddings
  );
  const maxDistractionSimilarityTitle = averageSimilarity(
    titleEmbeddings,
    distractionEmbeddings
  );
  const maxFocusSimilarityContent = maxSimilarity(
    contentEmbeddings,
    focusEmbeddings
  );
  ...
  console.log(
    'Max Focus Score (Title): ',
    maxFocusSimilarityTitle,
    'Max Focus Score (Content): ',
    maxFocusSimilarityContent,
    'Max Distraction Score: ',
    maxDistractionSimilarityTitle
  );
...
async function analyzeEmbeddingsAvg(titleEmbeddings, contentEmbeddings) {
  ...
  const distractionScore = averageSimilarity(
    titleEmbeddings,
    distractionEmbeddings
  );
...
async function embedText(text) {
  ...
  const embeddings = await loadedModel.embed([text]);
...
function averageSimilarity(embedding, exampleEmbeddings) {
  const similarities = exampleEmbeddings.map((example) =>
    cosineSimilarity(embedding, example)
  );
